...
- Ελάχιστη παραγγελία 8€ (setting min_order, Admin Settings) **μόνο για delivery** — υπολογίζεται σε προϊόντα μετά την έκπτωση, χωρίς delivery fee; η Παραλαβή δεν έχει ελάχιστο. Cart/Checkout notes με link «επίλεξε Παραλαβή». Κουμπί «Επίλεξε ζώνη ↑» στα totals του checkout κάνει scroll/focus στη ζώνη (highlight μέχρι επιλογή)
- Checkout: υποχρεωτικά Όνομα/Τηλέφωνο (10-15 ψηφία)/Email με labels, inline errors, missing-fields-note, γκρι κουμπί μέχρι να συμπληρωθούν; backend validation 400 στο POST /orders
- Email καταστήματος σε κάθε νέα παραγγελία (event store_new_order → SMTP_USERNAME ή STORE_NOTIFY_EMAIL, κουμπί «Άνοιγμα Admin» με PUBLIC_URL). Ανάλυση τιμής παντού: «Υποσύνολο προϊόντων» → «Κόστος delivery +X» / «Παραλαβή 0,00» → «Σύνολο» (Cart, Checkout, OrderConfirm, admin detail, ticket, emails)
- Email ειδοποιήσεις πελάτη (Gmail SMTP, backend/mailer.py, creds σε backend/.env SMTP_*): υποχρεωτικό email στο checkout; Αποδοχή στο admin ανοίγει ETA dialog (λεπτά) → status confirmed + eta_minutes/eta_at + email «έγινε αποδεκτή, ~Χ λεπτά»; email σε ready (μόνο παραλαβή) & delivering; log στη collection notifications (GET /api/admin/orders/{id}/notifications). Σελίδα /order/:id δείχνει ETA. Alarm: συνεχής ήχος (1.5s) όσο υπάρχουν «Νέες», σταματά με χειροκίνητη Αποδοχή. Admin PWA
- Admin PWA: /admin-manifest.json (start_url /admin/orders, standalone) εναλλάσσεται από AdminLayout, κουμπί «Εγκατάσταση στον υπολογιστή» (beforeinstallprompt), «Προβολή καταστήματος», badge ADMIN. Header: κουμπί «Admin Panel» όταν ο χρήστης είναι admin. README με οδηγίες εγκατάστασης/backup
- Αλοιφές: 5 σως όπως στο e-food (250cc, Σως chef 1,50 / Μαγιονέζα 1,50 / Ουγγαρέζα 2 / BBQ 2 / Caesar's 2), χωρίς ετικέτες νηστίσιμο. Κουμπί «Νηστίσιμο Μενού» στην αρχική εμφανίζει ολόκληρη την κατηγορία lent (14). Πληρωμή `card_pos` («Πληρωμή με κάρτα στο κατάστημα») μόνο για παραλαβή (backend guard). Admin Orders: ήχος νέας παραγγελίας (Web Audio, lib/sound.js) με toggle Ήχος ON/OFF
